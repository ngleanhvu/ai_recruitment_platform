...
public record CreateCandidateFromResumeUseCase (
    ...
    public void execute(CreateCandidateFromResumeRequest request) {
        ...
        List<Education> educations = request.educations().stream()
                .map(candidateMapper::toEducation)
                .toList();
        List<Experience> experiences = request.experiences().stream()
                .map(candidateMapper::toExperience)
                .toList();
        List<Skill> skills = request.skills().stream()
                .map(candidateMapper::toSkill)
                .toList();
        List<SocialLink> socialLinks = request.socialLinks().stream()
                .map(candidateMapper::toSocialLink)
                .toList();
        Candidate candidate = Candidate.create("", email.value(), profile.firstName(), profile.lastName(), profile.phone(), "");
