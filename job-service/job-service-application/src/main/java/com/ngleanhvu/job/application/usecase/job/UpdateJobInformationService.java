...
@Service
public record UpdateJobInformationService(JobRepository jobRepository, JobMapper jobMapper)
    implements UpdateJobInformationUseCase {
  public void execute(CreateJobRequest request, JobId jobId) {
    ...
    job.updateInformation(request.title(), request.description(), employmentType, workPlace);
