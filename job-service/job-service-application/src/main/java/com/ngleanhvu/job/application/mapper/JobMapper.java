...
public final class JobMapper {
  public Job toDomain(CreateJobRequest request) {
    if (ValidationUtil.isNull(request)) return null;
  ...
  public WorkPlace toWorkPlace(WorkPlaceRequest request) {
    return new WorkPlace(
        WorkplaceType.from(request.workplaceType()), this.toAddress(request.address()));
  }
  ...
  public JobRequirements toJobRequirements(JobRequirementRequest request) {
    List<SkillRequirement> skills =
        request.skills().stream().map(this::toSkillRequirement).toList();

    List<LanguageRequirement> languages =
        request.languages().stream().map(this::toLanguageRequirement).toList();
    ...
    return new JobRequirements(skills, experience, education, languages);
  }
