...
public class JobDocumentMapper {
  public JobDocument toDocument(Job job) {
    ...
    if (!ValidationUtil.isNull(job.getBenefits())) {
      List<String> benefits = job.getBenefits().stream().map(Benefit::name).toList();
    ...
    if (!ValidationUtil.isNull(job.getSalary())) jobDocument.setSalary(job.getSalary());
  ...
  public Job toDomain(JobDocument jobDocument) {
    List<Benefit> benefits = jobDocument.getBenefits().stream().map(Benefit::new).toList();
    ApplicationDeadline deadline =
        !ValidationUtil.isNull(jobDocument.getApplicationDeadline())
